...
function headerIndex(headers,names){
 const normalized=headers.map(normalize);for(const name of names){const index=normalized.findIndex(header=>header.includes(name));if(index>=0)return index}return-1;
}
